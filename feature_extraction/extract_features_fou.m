function [feature_map, img_samples] = extract_features_fou(image, pos, scales, features, gparams, extract_info, object_size, scales_p)
...
if num_scales==1
% cut=cv.grabCut();
for sam_ind=1:size(img_samples,1)
    samples_x=img_samples{sam_ind,1};
    cent_pos=size(samples_x(:,:,1))/2;
    pos_now=cent_pos-object_size([2,1])/2;
    rect_grab=[pos_now object_size([2,1])];
    
    mask=cv.grabCut(samples_x,rect_grab);
%     mask=cv.compare(mask,3,'eq');
    img_samples_temp=cv.copyTo(samples_x,'Mask',mask);
%     img_samples_temp(find(img_samples_temp==0))=255;
    img_samples{sam_ind,1}=img_samples_temp;
end
%     save('C:\Users\Dominik\Desktop\lea\img_sample\img_sample.mat','img_samples');
end

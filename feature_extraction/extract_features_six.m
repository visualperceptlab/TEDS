function [feature_map, img_samples] = extract_features_six(image, pos, scales, features, gparams, extract_info, scales_p, object_size)
...
%             scas_y_fli=fliplr(scas_y_p);
            scas_y_fli=circshift(scas_y_p,[0,floor(length(scas_y_p)/2)]);
...
%             scas_x_fli=fliplr(scas_x_p);
            scas_x_fli=circshift(scas_x_p,[0,floor(length(scas_x_p)/2)]);
...
%     gparams.cell_size = feat.fparams.cell_size;
    ...
    if feat.is_cell
        ...
        feature_map(ind:ind+num_blocks-1) = feat.getFeature(img_samples{img_sample_ind}, feat.fparams, gparams, object_size);
    else
        num_blocks = 1;

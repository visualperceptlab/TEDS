function descriptors = do_descriptor2(octave, ... % 一组的高斯尺度空间
...
for p = 1: key_num  %对各个关键点处理
    ...
    W =   floor( 0.8 * SBP * (NBP + 2) / 1.0 + 0.5);
    
    descriptor01 = zeros(1,8, NBO);
    descriptor02 = zeros(NBP,NBP, NBO);
...
%           angle = mod(-angle + theta0, 2*pi);      % 用关键点的主方向调整角度 并且 mod it with 2*pi
 ...
 %%%%%%%%%%%%%%%%%%%
            nr=sqrt(dx^2+dy^2);
            nc=atan(dy/dx)-theta0;
            nx = ( costh0 * dx + sinth0 * dy) / (SBP*1.5) ; % nx 是旋转(dx, dy)后的规格化位置 with the major orientation angle. this tells which x-axis spatial bin the pixel falls in 
            ny = (-sinth0 * dx + costh0 * dy) / (SBP*1.5) ; 
           ...
           for rr=1:3
               for dbint = 0:1  
                  ...
                  ddy=mod(ddy,8)+1; 
                   ...
                   if (ddx==1||ddx==2)&& ( isnan(bint) == 0) 
                         weight = wincoef * mag *(1/ddx) * (1/(abs(rr-2)+1)) * abs(1 - dbint - rbint) ;
                            
   
                              descriptor01(ddx,ddy, mod((bint+dbint),NBO)+1) = ...
                                  descriptor01(ddx, ddy, mod((bint+dbint),NBO)+1 ) +  weight ;
                   end
               end
           end
           for(dbinx = 0:1) 
                   ...
                   for(dbint = 0:1) 
                        ...
                        if( binx+dbinx >= -(NBP/2) && ...
                            ...
                            biny+dbiny <   (NBP/2) &&  isnan(bint) == 0) 
                              ...
                              weight1 = wincoef * mag * abs(1 - dbinx - rbinx) ...
                                  * abs(1 - dbiny - rbiny) ...
                                  * abs(1 - dbint - rbint) ;
   
                              descriptor02(binx+dbinx + NBP/2 + 1, biny+dbiny + NBP/2+ 1, mod((bint+dbint),NBO)+1) = ...
                                  descriptor02(binx+dbinx + NBP/2+ 1, biny+dbiny + NBP/2+ 1, mod((bint+dbint),NBO)+1 ) +  weight1 ;
                        end
                   end
               end
            end
    ...
    descriptor11 = reshape(descriptor01, 1, 1 * 8 * NBO);%用一维向量表示各梯度值
    descriptor12 = reshape(descriptor02, 1, NBP * NBP* NBO);%用一维向量表示各梯度值
    descriptor=[descriptor11 descriptor12];

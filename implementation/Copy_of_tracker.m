...
%             [xl, img_samples] = extract_features(im, sample_pos, sample_scale, features, global_fparams, feature_extract_info);
            [xl, img_samples] = extract_features_dou(im, sample_pos, sample_scale, features, global_fparams, feature_extract_info);
            ...
            if sind<8
            trans_row_v=[disp_row, disp_col] .* (img_support_sz./output_sz) * currentScaleFactor(1) * scaleFactors(sind);
            trans_col_v=[disp_row, disp_col] .* (img_support_sz./output_sz) * currentScaleFactor(2) * scaleFactors(sind);
             scale_change_factor = [scaleFactors(sind) scaleFactors(sind)];
            elseif sind<15
            trans_row_v=[disp_row, disp_col] .* (img_support_sz./output_sz) * currentScaleFactor(1) * scaleFactors(fix_ind);
            trans_col_v=[disp_row, disp_col] .* (img_support_sz./output_sz) * currentScaleFactor(2) * scaleFactors(sind-7);  
             scale_change_factor = [scaleFactors(fix_ind) scaleFactors(sind-7)];
            elseif sind<22
            trans_row_v=[disp_row, disp_col] .* (img_support_sz./output_sz) * currentScaleFactor(1) * scaleFactors(sind-14);
            trans_col_v=[disp_row, disp_col] .* (img_support_sz./output_sz) * currentScaleFactor(2) * scaleFactors(fix_ind);
             scale_change_factor = [scaleFactors(sind-14) scaleFactors(fix_ind)];
            end
            translation_vec = [trans_col_v(1), trans_row_v(2)];
...
%         end
        ...
        global_fparams.augment = 1;
        sample_pos = round(pos);
        [xl, img_samples] = extract_features_dou(im, sample_pos, currentScaleFactor, features, global_fparams, feature_extract_info);
        % do windowing of features
        xlw = cellfun(@(feat_map, cos_window) bsxfun(@times, feat_map, cos_window), xl, cos_window, 'uniformoutput', false);
        % compute the fourier series
        xlf = cellfun(@fft2, xlw, 'uniformoutput', false);
...
%     target_sz = base_target_sz * currentScaleFactor;
    ...
    target_sz=[target_sz_1(1) target_sz_2(2)];
    else
        target_sz = base_target_sz * currentScaleFactor;     
    end    
...
%             resp_sz = round(img_support_sz*currentScaleFactor*scaleFactors(sind));
            ...
            resp_sz_y = round(img_support_sz.*currentScaleFactor(2)*scaleFactors(sind-5));
            elseif sind<16
            resp_sz_x = round(img_support_sz.*currentScaleFactor(1)*scaleFactors(sind-10));
